...
const Achievements: React.FC<AchievementsProps> = ({ gameState }) => {
  const badgesWithUnlockStatus = BADGES.map(badge => {
    let isLocked = true;

    if (badge.id === 'b1') {
      isLocked = gameState.completedStories.length < 1;
    } else if (badge.id === 'b2') {
      isLocked = gameState.readingStreak < 7;
    } else if (badge.id === 'b3') {
      isLocked = gameState.completedStories.length < 5;
    }
    ...
    <div className="flex flex-col min-h-screen bg-bg-dark bg-star-dust pb-24">
      ...
      <div className="flex gap-4 p-4">
         <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm">
             ...
             <p className="text-3xl font-bold text-white">{gameState.totalStoriesRead}</p>
         </div>
         <div className="flex-1 bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm">
             <div className="flex items-center gap-2 mb-2">
             ...
             <p className="text-3xl font-bold text-white">{gameState.sleepScore}</p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 p-4">
          {badgesWithUnlockStatus.map(badge => (
